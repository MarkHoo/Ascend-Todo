  ...
  const handleCalendarDrop = async (event: React.DragEvent, date: string, hour = 9, minute = 0) => {
    ...
    try {
      if (taskId) {
        ...
        await moveEntryAt(JSON.parse(entryRaw) as CalendarEntry, date, hour, snappedMinute);
      }
    } catch (error) {
      toast.error(String(error));
    }
            ...
            return (
              <div key={`events-${key}`} className="relative border-r border-border">
                {layoutTimedEntries(timed, hours[0], hours[hours.length - 1] + 1, hourRowHeight).map((item) => (
                  <TimedEventBlock key={item.entry.id} item={item} onClick={() => onOpenEntry(item.entry)} />
                ))}
              </div>
            );
          })}
...
function TimedEventBlock({ item, onClick }: { item: TimedEntryLayout; onClick: () => void }) {
  const { entry } = item;
