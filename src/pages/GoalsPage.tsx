...
const krToDraft = (kr: KeyResult): DraftKR => ({
  ...
  weight: kr.weight,
  checkDate: formatForDateTimeInput(kr.checkDate),
});
      ...
      for (const kr of validKRs) {
        if (kr.id) {
          await keyResultsApi.update({
            ...
            weight: kr.weight,
            checkDate: dateTimeInputToIso(kr.checkDate) || null,
          });
        } else {
          await createKeyResult({
            ...
            weight: kr.weight,
            checkDate: dateTimeInputToIso(kr.checkDate),
          });
        }
      }
    ...
    >
      <div className="space-y-5">
        ...
        <section className="border-t border-border pt-4">
          ...
          <div className="space-y-3 max-h-[46vh] overflow-y-auto overflow-x-hidden pr-2">
...
function DraftKRRow({ kr, isLast, onChange, onDelete }: {
  ...
  return (
    <div className="border-b border-border pb-3">
      ...
      {!collapsed && <div className="mt-2 ml-9 flex items-center gap-4 whitespace-nowrap pb-1">
        <select className="input h-10 w-36 shrink-0" value={kr.type} onChange={(event) => onChange({ type: event.target.value as KRType })}>
        ...
        {kr.type === 'metric' ? (
          <>
            <label className="flex items-center gap-2 text-sm shrink-0">
              ...
              <input className="input h-10 w-24" value={unit} onChange={(event) => onChange({ unit: event.target.value })} />
            </label>
            <label className="flex items-center gap-2 text-sm shrink-0">
              <span className="text-text-muted">{t('goal.startValue')}:</span>
              <span className="relative inline-flex">
                <input className="input h-10 w-32 pr-14" type="number" value={kr.startValue} onChange={(event) => onChange({ startValue: Number(event.target.value) || 0 })} />
            ...
            <label className="flex items-center gap-2 text-sm shrink-0">
              ...
              <span className="relative inline-flex">
                <input className="input h-10 w-32 pr-14" type="number" value={kr.targetValue} onChange={(event) => onChange({ targetValue: Number(event.target.value) || 0 })} />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 max-w-12 truncate text-xs text-text-muted border-l border-border pl-2">{unit}</span>
              </span>
            </label>
          </>
        ...
        <div className="flex w-52 shrink-0 items-center gap-2 text-sm">
          <span className="text-text-muted">{t('goal.checkDate')}:</span>
          <NativeDateTimeInput
            value={kr.checkDate}
            onChange={(value) => onChange({ checkDate: value })}
...
function GoalDetailModal({
  ...
  return (
    <Modal open={true} onClose={onClose} size="goalWide">
...
function KeyResultsPanel({
  ...
  const gridClass = isFinished
    ? 'grid grid-cols-[minmax(360px,1fr)_210px_70px] items-center gap-3'
    : 'grid grid-cols-[minmax(360px,1fr)_210px_70px_104px] items-center gap-3';
  return (
    <>
          ...
          return (
            <div key={kr.id} className="border-b border-border py-3">
              <div className={gridClass}>
                <button className="min-w-0 font-medium flex items-center gap-2 text-left hover:text-primary" onClick={() => setCollapsed((state) => ({ ...state, [kr.id]: !isCollapsed }))}>
                  {isCollapsed ? <ChevronRight size={15} className="text-text-muted" /> : <ChevronDown size={15} className="text-text-muted" />}
                  <span className="min-w-0 flex-1 break-words leading-5">{kr.title}</span>
                  {kr.checkDate && (
