...
#[tauri::command]
pub fn delete_manual_calendar_event(state: State<DbState>, id: String) -> AppResult<()> {
    let c = conn(&state);
    let existing: Option<(String, i64)> = c
        .query_row(
            "SELECT source_type, readonly FROM calendar_events WHERE id = ?",
    ...
    if source_type != "manual" || readonly != 0 {
        return Err(AppError::Invalid(
            "Only local schedules can be deleted".into(),
        ));
    }
    c.execute("DELETE FROM calendar_events WHERE id = ?", params![id])?;
    Ok(())
}
