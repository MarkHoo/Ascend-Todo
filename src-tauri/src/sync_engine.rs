...
use crate::models::{
    Board, CheckIn, Goal, List, Milestone, PomodoroSession, Snapshot, Subtask, Task, UserProfile,
};
...
pub fn build_snapshot(c: &Connection) -> AppResult<Snapshot> {
    ...
    {
        let mut stmt = c.prepare(
            "SELECT id, task_id, title, is_completed, position, created_at FROM subtasks",
        )?;
        for r in stmt.query_map([], |r| {
            Ok(Subtask {
                id: r.get(0)?,
...
pub fn apply_snapshot(c: &Connection, s: &Snapshot) -> AppResult<()> {
    ...
    for s in &s.subtasks {
        tx.execute(
            "INSERT INTO subtasks (id, task_id, title, is_completed, position, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            params![s.id, s.task_id, s.title, s.is_completed as i64, s.position, s.created_at],
        )?;
    }
    for g in &s.goals {
        tx.execute(
